...
X_train, X_test, Y_train, Y_test = train_test_split(X, Y, test_size=0.5, shuffle=False)"""
    ...
    if i != 0:
        print(i)
        Y_Mean_1 = mean(Y_temp.index[:i - 1])
        Y_Mean_2 = mean(Y_temp.index[i:])
        P_Mean_1 = mean(P_temp.index[:i - 1])
        P_Mean_2 = mean(P_temp.index[i:])
        ...
        print("MAE: ", mean_absolute_error(Y_Mean_1, P_Mean_1))
        print("MAE: ", mean_absolute_error(Y_Mean_2, P_Mean_2))

        # media delle differenze al quadrato tra previsioni e target.
        print("MSE: ", mean_squared_error(Y_Mean_1, P_Mean_1))
        print("MSE: ", mean_squared_error(Y_Mean_2, P_Mean_2))

        # proporzione tra variabilità e correttezza dei dati del modello.
        print("R2 test: ", r2_score(Y_Mean_1, P_Mean_1))
        print("R2 test: ", r2_score(Y_Mean_2, P_Mean_2))

        # misura di quanto i valori stimati si discostano dai valori reali
        print("Max Error: ", max_error(Y_Mean_1, P_Mean_1))
        print("Max Error: ", max_error(Y_Mean_2, P_Mean_2))
    else:
        ...
        # proporzione tra variabilità e correttezza dei dati del modello.
        print("R2 test: ", r2_score(Y_Mean, P_Mean))
...
print("R2 test: ", r2_score(Y_Total_Mean, P_Total_Mean))
...
# grafico:
"""with open('2GroupExperiment/Experiment2Futsal.csv', 'a') as f:
    wtr = csv.writer(f)
    wtr.writerow(Y_test)
    wtr.writerow(P_test)"""
