    ...
    for pTrain in range(1, 6):
        if pTrain == p:
            ...
            """else:
                ...
                if count == 0 or countP == 0:
                    with open('DividedTimeFeatures/' + path + '/P' + str(p) + '/training.csv', 'w',
                              newline='') as f:
                        data.to_csv(f, index=False)
                        if countP == 0:
                            countP = 1
                else:
                    with open('DividedTimeFeatures/' + path + '/P' + str(p) + '/training.csv', 'a',
                              newline='') as f:
                        data.to_csv(f, header=False, index=False)
                count += 1
    else:
        ...
        for pTrain in range(1, 6):
            if pTrain == p:
                ...
                data.to_csv('DividedTimeFeatures/' + path + '/P' + str(p) + '/test' + '.csv', index=False)
            else:
                data = pd.read_csv(fileName + str(pTrain) + '/Exercise1' + ".csv")
                if count == 0 or countP == 0:
                    with open('DividedTimeFeatures/' + path + '/P' + str(p) + '/training.csv', 'w',
