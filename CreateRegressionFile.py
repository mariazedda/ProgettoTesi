...
if sport == "c":
    fileName = 'DividedFeatures\\calcio\\P'
    path = 'calcio'
else:
    fileName = 'DividedFeatures\\futsal\\P'
    path = 'futsal'
    ...
    if p != 4:
        ...
        for pTrain in range(1, 6):
            if pTrain == p:
                for s in range(1, 3):
                    ...
                    data.to_csv('Average/' + path + '/P' + str(p) + '/test' + str(s) + '.csv',
                                index=False)
            else:
                if pTrain != 4:
                    for s in range(1, 3):
                        ...
                        if count == 0 or countP == 0:
                            ...
                            with open('DividedTestTrainingFile/' + path + '/P' + str(p) + '/training.csv', 'a', newline='') as f:
                    ...
                    if count == 0 or countP == 0:
                        with open('DividedTestTrainingFile/' + path + '/P' + str(p) + '/training.csv', 'w',
                                  newline='') as f:
                            data.to_csv(f, index=False)
                            if countP == 0:
                                countP = 1
                    else:
                        with open('DividedTestTrainingFile/' + path + '/P' + str(p) + '/training.csv', 'a',
                                  newline='') as f:
                            data.to_csv(f, header=False, index=False)
                        count += 1
    else:
        ...
        for pTrain in range(1, 6):
            if pTrain == p:
                ...
                data.to_csv('DividedTestTrainingFile/' + path + '/P' + str(p) + '/test' + '.csv', index=False)
            else:
                data = pd.read_csv(fileName + str(pTrain) + '\\Exercise1' + ".csv")
                if count == 0 or countP == 0:
                    with open('DividedTestTrainingFile/' + path + '/P' + str(p) + '/training.csv', 'w',
